...
static void
WindowSizeCallback(GLFWwindow* window, int width, int height); // GLFW3 WindowSize Callback, runs when window is resized
...
static void
WindowFocusCallback(GLFWwindow* window, int focused); // GLFW3 WindowFocus Callback, runs when window get/lose focus
static void WindowDropCallback(
    GLFWwindow* window, int count, const char** paths
); // GLFW3 Window Drop Callback, runs when drop files into window
static void WindowContentScaleCallback(
    GLFWwindow* window, float scalex, float scaley
); // GLFW3 Window Content Scale Callback, runs when window changes scale
...
static void
MouseCursorPosCallback(GLFWwindow* window, double x, double y); // GLFW3 Cursor Position Callback, runs on mouse move
static void MouseScrollCallback(
    GLFWwindow* window, double xoffset, double yoffset
); // GLFW3 Scrolling Callback, runs on mouse wheel
static void
CursorEnterCallback(GLFWwindow* window, int enter); // GLFW3 Cursor Enter Callback, cursor enters client area
    ...
    if (!CORE.Window.fullscreen) {
        ...
        if (monitor == NULL) {
            ...
            glfwSetWindowMonitor(
                platform.handle, NULL, 0, 0, CORE.Window.screen.width, CORE.Window.screen.height, GLFW_DONT_CARE
            );
        } else {
            ...
            glfwSetWindowMonitor(
                platform.handle, monitor, 0, 0, CORE.Window.screen.width, CORE.Window.screen.height, GLFW_DONT_CARE
            );
        }
    ...
    if ((monitor >= 0) && (monitor < monitorCount)) {
        ...
        if (mode) {
            if (!IsWindowState(FLAG_BORDERLESS_WINDOWED_MODE)) {
                ...
                // NOTE: The order matters here, it must set size first, then set position, otherwise the screen will be
...
void SetWindowState(unsigned int flags)
{
    ...
    // NOTE: This must be handled before FLAG_FULLSCREEN_MODE because ToggleBorderlessWindowed() needs to get some
    // fullscreen values if fullscreen is running
    if (((CORE.Window.flags & FLAG_BORDERLESS_WINDOWED_MODE) != (flags & FLAG_BORDERLESS_WINDOWED_MODE)) &&
    ...
    if (((CORE.Window.flags & FLAG_WINDOW_MOUSE_PASSTHROUGH) != (flags & FLAG_WINDOW_MOUSE_PASSTHROUGH)) &&
...
void SetWindowMonitor(int monitor)
{
    ...
    if ((monitor >= 0) && (monitor < monitorCount)) {
        if (CORE.Window.fullscreen) {
            TRACELOG(
                LOG_INFO, "GLFW: Selected fullscreen monitor: [%i] %s", monitor, glfwGetMonitorName(monitors[monitor])
            );

            const GLFWvidmode* mode = glfwGetVideoMode(monitors[monitor]);
            glfwSetWindowMonitor(
                platform.handle, monitors[monitor], 0, 0, mode->width, mode->height, mode->refreshRate
            );
        } else {
            ...
            glfwGetMonitorWorkarea(
                monitors[monitor], &monitorWorkareaX, &monitorWorkareaY, &monitorWorkareaWidth, &monitorWorkareaHeight
            );

            // If the screen size is larger than the monitor workarea, anchor it on the top left corner, otherwise,
            // center it
...
void SetClipboardText(const char* text) { glfwSetClipboardString(platform.handle, text); }

// Get clipboard text content
// NOTE: returned string is allocated and freed by GLFW
const char* GetClipboardText(void) { return glfwGetClipboardString(platform.handle); }
...
int SetGamepadMappings(const char* mappings) { return glfwUpdateGamepadMappings(mappings); }
...
const char* GetKeyName(int key) { return glfwGetKeyName(key, glfwGetKeyScancode(key)); }
...
void PollInputEvents(void)
{
    ...
    for (int i = 0; i < MAX_GAMEPADS; i++) {
        if (CORE.Input.Gamepad.ready[i]) // Check if gamepad is available
        {
            ...
            glfwGetGamepadState(
                i, &state
            ); // This remapps all gamepads so they have their buttons mapped like an xbox controller
            ...
            CORE.Input.Gamepad.currentButtonState[i][GAMEPAD_BUTTON_LEFT_TRIGGER_2] =
                (char)(CORE.Input.Gamepad.axisState[i][GAMEPAD_AXIS_LEFT_TRIGGER] > 0.1f);
            CORE.Input.Gamepad.currentButtonState[i][GAMEPAD_BUTTON_RIGHT_TRIGGER_2] =
                (char)(CORE.Input.Gamepad.axisState[i][GAMEPAD_AXIS_RIGHT_TRIGGER] > 0.1f);
    ...
    if ((CORE.Window.eventWaiting) ||
        (IsWindowState(FLAG_WINDOW_MINIMIZED) && !IsWindowState(FLAG_WINDOW_ALWAYS_RUN))) {
...
int InitPlatform(void)
{
    ...
    */
    ...
    // additionally auto iconify restores the hardware resolution of the monitor if the window that loses focus is a
    // fullscreen window
    ...
    if ((CORE.Window.flags & FLAG_WINDOW_HIGHDPI) > 0) {
        ...
        // Windows and X11. On platforms like macOS the resolution of the framebuffer is changed independently of the
        // window size.
        glfwWindowHint(
    ...
    if (rlGetVersion() == RL_OPENGL_21) {
        ...
        ); // Profiles Hint: Only 3.3 and above!
    ...
    // functions. Forcing this initialization here avoids doing it on PollInputEvents() called by EndDrawing() after
    // first frame has been just drawn. The initialization will still happen and possible delays still occur, but before
    // the window is shown, which is a nicer experience. REF: https://github.com/raysan5/raylib/issues/1554
    ...
    if (CORE.Window.fullscreen) {
        ...
        if ((CORE.Window.screen.height == CORE.Window.display.height) &&
            (CORE.Window.screen.width == CORE.Window.display.width)) {
            // If screen width/height equal to the display, we can't calculate the window pos for toggling
            // full-screened/windowed. Toggling full-screened/windowed with pos(0, 0) can cause problems in some
        ...
        // Obtain recommended CORE.Window.display.width/CORE.Window.display.height from a valid videomode for the
        // monitor
        int count = 0;
        ...
        TRACELOG(
            LOG_INFO,
            "SYSTEM: Closest fullscreen videomode: %i x %i",
        ...
        // TODO: It is a quite cumbersome solution to display size vs requested size, it should be reviewed or
        // removed... HighDPI monitors are properly considered in a following similar function: SetupViewport()
        ...
        // glfwSetWindowMonitor(platform.handle, glfwGetPrimaryMonitor(), 0, 0, CORE.Window.screen.width,
        ...
        platform.handle = glfwCreateWindow(
            creationWidth, creationHeight, (CORE.Window.title != 0) ? CORE.Window.title : " ", NULL, NULL
        );
    ...
    if ((result != GLFW_NO_WINDOW_CONTEXT) && (result != GLFW_PLATFORM_ERROR)) {
        ...
        if ((CORE.Window.flags & FLAG_WINDOW_HIGHDPI) > 0) {
            ...
            CORE.Window.screenScale = MatrixScale(
                (float)fbWidth / CORE.Window.screen.width, (float)fbHeight / CORE.Window.screen.height, 1.0f
            );
    ...
    if (!CORE.Window.ready) {
        ...
        // Here CORE.Window.render.width/height should be used instead of CORE.Window.screen.width/height to center the
        // window correctly when the high dpi flag is enabled.
